...
struct SwiftNominalIdentitySuite {
  ...
  @Test func moduleQualificationDistinguishesSameLeafName() {
  ...
  @Test func nestedNamesKeepTheirParentChain() {
  ...
  @Test func equalIdentitiesHashTogether() {
...
struct CrossModuleExtensionsCollectionSuite {
  ...
  @Test func existingAnalysesRecordNothing() throws {
  ...
  @Test func emptyCollectionQueriesReturnEmpty() {
...
struct TypesConformingSuite {
  ...
  /// The motivating shape, reduced. A framework may state a conformance to a refining
  /// protocol rather than to the one a consumer asks about, so the query must close over
  /// protocol inheritance. This mirrors FoundationModels, where `Int` states only
  /// `Generable` yet is usable wherever `PromptRepresentable` is expected
  @Test func conformanceIsFoundThroughProtocolInheritance() throws {
    let result = try analyze(
      sources: [
        (
          "/fake/Source.swift",
  ...
  @Test func directConformanceIsReportedAsStated() throws {
    let result = try analyze(
      sources: [
        (
          "/fake/Source.swift",
  ...
  @Test func unrelatedProtocolYieldsNoConformers() throws {
    let result = try analyze(
      sources: [
        (
          "/fake/Source.swift",
  ...
  /// A protocol refining itself through a cycle is not valid Swift, but malformed or
  /// partially-resolved input can produce one. The walk must terminate rather than spin
  @Test func cyclicRefinementTerminates() throws {
    let result = try analyze(
      sources: [
        (
          "/fake/Source.swift",
  ...
  /// Protocols are not conformers of themselves for this query's purposes: asking what
  /// conforms to a protocol should surface concrete types, not its own refinements
  @Test func refiningProtocolsAreNotReportedAsConformers() throws {
    let result = try analyze(
      sources: [
        (
          "/fake/Source.swift",
