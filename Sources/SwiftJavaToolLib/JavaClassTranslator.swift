...
extension JavaClassTranslator {
  ...
  package func renderConstructor(
    ...
    return """
      \(raw: failedImportPrefix)\(raw: availableAttributes.render())@JavaMethod
  ...
  package func renderField(
    ...
    if !translationErrors.hasErrors, let optionalType = typeName.optionalWrappedType() {
      ...
      return """
        \(raw: failedImportPrefix)\(raw: availableAttributes.render())\(fieldAttribute)(isFinal: \(raw: javaField.isFinal))
        public var \(raw: swiftFieldName): \(raw: typeName)
        """
    }
  }
