...
concrete value together with its type metadata, and dispatches each protocol
requirement through a dedicated native thunk that reconstructs the existential from that
value. 

The example below shows how different "`any Greeter` returning methods,
retain their expected runtime behavior in practice:
